...
class TrkAnaPlots {
  ...
  TrkAnaPlots(TTree* tn, TTree* tp, float momwin=1.5) : _tn(tn), _tp(tp) {
    BuildCuts(momwin);
...
void TrkAnaPlots::TrkCaloHit(int pdg) {
  char cstring[100];
//  snprintf(cstring,100,"detch.active&&dequal.TrkQual>%f&&abs(demc.pdg)==%i&&demcxit.momz>0",tqcut,pdg);
  snprintf(cstring,100,"detch.active&&abs(demc.pdg)==%i&&demcxit.momz>0",pdg);
